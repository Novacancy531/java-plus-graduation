...
import ru.practicum.error.ErrorMessage;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorMessage> handle(HandlerMethodValidationException ex) {

        var details = ex.getAllErrors().stream()
                .map(e -> e.getClass() + ": " + e.getDefaultMessage())
                .toList();

        log.warn("Method validation failed: {}", details);
        return ResponseEntity.badRequest()
                .body(new ErrorMessage("Validation failure: " + String.join("; ", details)));
    }
